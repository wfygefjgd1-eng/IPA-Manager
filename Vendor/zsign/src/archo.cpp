	...
	string& strOutput)
{
	...
	// Extension binaries are signed with the profile's entitlements VERBATIM
	// (application-identifier stays at the main app id). History of on-device
	// experiments: every variant that rewrote either the embedded profile's or
	// the binary's application-identifier to the extension's own id failed to
	// get the extensions registered on iOS 27 (entries never enumerated in the
	// share sheet). The remaining untested combination is the plainest one -
	// both profile and binary carry the app id, exactly what stock zsign
	// produces - so the id rewrite is reverted. App groups and the team prefix
