...
- **v1.0.183**（一条龙进度弹窗版）：① 导入/签名/安装两张独立进度卡合并为一张胶囊弹窗（仍统一挂 ContentView 中央 overlay）：新增跨阶段实时总百分比——导入 0–50%（解压按真实字节）、签名 50–95%（zsign 回报进度）、发起安装 95–100%（本地服务器/manifest 无可靠进度，按阶段定性推进到满格；系统安装确认弹窗由 iOS 接管不计入），左侧圆环随整体进度描边、百分比居中，渐变进度条带 50%/95% 阶段分界刻度；「导入 → 签名 → 安装」三步指示器（完成打勾/进行中高亮/未开始置灰），无可靠进度的阶段以转圈示意，百分比不再停滞。② 新增已用时间：从导入开始计时、精确到 0.1 秒实时跳动（TimelineView 周期刷新，只重绘时间文本、不产生额外 @Published 状态流），随弹窗一起消失；计时起点 pipelineStartedAt 生命周期覆盖导入→签名→安装全程——签名/安装接管不重置，批量导入/多文件衔接经 0.5 秒延迟复查防中途清零，弹窗收场（"安装已发起"停留 6 秒或失败 toast）即清除。③ 交接防闪修复：导入成功→签名开始的交接此前跨两个主线程事务（导入卡消失、流水线卡出现之间隔一帧，弹窗闪空），setPipelineStatus 主线程同步赋值让 SwiftUI 把两次变化合并为一次更新；AutoPipelineStatus 增加显式 Stage 枚举（signing/installLaunching/installInitiated）供弹窗步骤落点判定，不再依赖阶段文案字符串匹配。
