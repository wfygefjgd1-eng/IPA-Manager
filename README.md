...
- **系统分享接收（双扩展）**：分享面板应用行「IPA Manager」入口（分享扩展）+ 动作区「IPA Manager」入口（动作扩展，类似"QQ闪传分享"），任意 App 里分享 .ipa/.zip/证书文件原地接收，打开 App 自动导入并签名安装；同时保留文档类型投递（「拷贝到 IPA Manager」）多通道。
...
- **仅构建 libcrypto**（3.x 的 Makefile 无 `build_libcrypto` 目标，脚本实际执行 `make build_generated` + `make libcrypto.a`）：全库无任何 SSL_*/TLS_* 调用，不构建也不链接 libssl（工程链接项只有 `libcrypto-device.a`），节省约 25-30% 编译时间；
...
- **v1.0.178**（导入解析性能优化版）：① **直连 .ipa 导入/已签应用刷新改走中央目录轻量解析（最大单项 IO 优化）**：旧路径为读 Info.plist + 提图标把整个 IPA 解压成 GB 级副本再删除（2GB 包导入 = 2GB 纯浪费的写+读+删；已签应用每个新产物首扫同代价）。新路径只从 zip 中央目录单条目解出 Info.plist 与图标文件（毫秒级、零整包副本），Info.plist 条目匹配、图标候选收集、高分屏排序、icon 启发式兜底全部与整包解析同口径（InfoPlistParser 的候选/排序逻辑抽为共享静态方法供两路共用）；任何一步失败自动回退整包解析——最坏情况 = 旧行为，零风险。zip 输入（内嵌 ipa/.app 重打包）仍走完整管线不受影响。② 回前台投递扫描防抖：scenePhase 与 applicationDidBecomeActive 几乎同时各触发一次全量扫描（主线程目录枚举，Documents 子目录文件多时可观），0.8 秒窗口内的重复触发直接跳过；日志页"立即扫描"与 ipamanager:// 唤起传 force 不受限，延迟复查（2.5s/10s）间距大于窗口不受影响。③ saveState 移除下载任务的冗余全量重编码：DownloadManager 在每次任务变更时已自行持久化，AppState 每次导入/签名/证书变更再编码一次（含 resumeData 的任务可达数百 KB）纯属重复主线程 IO，两模块持久化职责解耦。
- **v1.0.176**（遗留权衡项清零版）：上一轮审查标记"暂不改动"的遗留项全部修复。① 安装服务器空闲判定重构（并发安装防互踩的补全）：45 秒死会话解锁此前是无差别的——用户在系统安装确认弹窗停留超过 45 秒时（manifest 已拉取、正等用户点"安装"），下一个并发/批量安装会把该会话当"死会话"杀掉服务器，用户随后点"安装"必然失败。现区分两种会话：SpringBoard 从未前来（零活动）的会话维持 45 秒解锁（保留 v1.0.118 修复的"死会话卡死 15 分钟"场景）；已有活动（弹窗停留）的会话在安装会话窗口内（10 分钟，与回前台/保活判定一致）保持忙，超窗仍无活动才解锁。EOF 5 秒解锁不变（IPA 已完整送达，随时可安全重启）。② 本地服务器新增每连接"请求头空闲看门狗"：监听器安装期绑定全部接口，扫描器/探测包建连后静默滞留会无限占用连接与描述符、挤占 SpringBoard 的真实安装连接——30 秒未送出完整请求头即断开，请求进入响应/流式阶段即解除，stop 全量清理。③ 下载续传一致性校验兼容 URLSession 的两种上报语义：旧实现只认"期望字节=剩余量"（offset+expected==total），若系统按"期望字节=全量"语义上报（不同系统版本/响应形态行为不一致），每次续传都被误判为内容变化而整包重下，断点续传形同虚设；新判定同时接受剩余量语义（offset+expected==total）与全量语义（expected==total 且 written≥断点偏移），两者都不满足（真正换源/服务器忽略 Range）才整包重下——任何语义下都不会比旧行为更差。④ 图标 mtime 记忆字典超过 512 条时按 5s TTL 剪枝（原条目从不移除、随会话无限增长）。
- **v1.0.175**（第六轮代码审查修复版）：全库复查后修复 5 处——① 签名引擎（关键）：ECC（P-256/P-384/P-521）证书签名 100% 失败的根因修复——`pkcs8DER` 重组 PKCS#8 时外层 PrivateKeyInfo 的 version 字节误编码为 1（标准 RFC 5958 要求 0，与 SEC1 内层的 version=1 混淆），OpenSSL `PEM_read_bio_PrivateKey` 解析直接拒绝，EC 证书一律失败于"签名初始化失败：证书或描述文件无效| 私钥为 PEM 格式"；RSA 路径不受影响。已用 OpenSSL 3.x 对 version=0/1 两种 DER 实测验证（v1 报 Invalid key、v0 解析成功且私钥值一致）。② 下载：修复"暂停后立即点继续"被静默吞掉的竞态——cancel(byProducingResumeData:) 的收尾回调到达前，"继续"落在已取消的旧任务上是 no-op，随后暂停回调把状态定回 .paused；现登记"继续"意图，暂停回调改走重建路径（有断点续传、无断点整包重下）。③ 下载：修复续传一致性校验触发整包重下时若 URL 失效，任务永久卡"下载中"（无 sessionTask 的僵尸态）；补失败收尾。④ 下载：任务终态（完成/失败/删除/重建）清理过期暂停标记与进度节流字典——旧任务被 cancel 后再点"暂停"是 no-op 取消（无新回调），过期标记会让新任务随后的一次失败被误判成"暂停完成"；节流字典原随任务数无限增长。⑤ 证书：修复遗留钥匙串孤儿清理的 no-op——Keychain 查询对 kSecAttrLabel 是精确匹配，带前缀的查询永远查不到"前缀+UUID 后缀"的孤儿条目，改为取回该 class 全部条目后按前缀过滤逐个删除。
- **v1.0.151**（统一接收队列版）：分享接收链路补上任务化架构。① **ImportPipeline 统一模块**（`Sources/ImportPipeline/`）：`ImportTask` 十一态状态机（pending→copying→copied→processing→extracting→parsing→signing→signed→installing→completed/failed）+ `ImportTaskStore`（App Group 扇出/扇入的任务 JSON 存取）+ `IncomingFileScanner`（主 App 认领器）；② **UUID 化接收存储**：扩展落盘文件改为 `Incoming/<UUID>-原名`（防同名覆盖/特殊字符/路径冲突），并紧随其后写入 `Incoming/Tasks/<UUID>.json` 任务记录（含原文件名/存储名/类型/时间/状态），**读回验证通过后扩展才结束请求**——"文件还没落盘就 completeRequest 导致偶发丢文件"从结构上不可能再发生；③ **主 App 扫描认领**：启动/回前台/手动扫描统一经 `IncomingFileScanner.scanAndClaim()` 认领 pending 与滞留 processing（上次进程死亡）的任务，交给既有统一导入入口，结算钩子回写任务终态（completed/failed+原因），扩展与主 App 不同时存活也不丢任务；④ 投递日志页新增"最近导入任务"卡（状态/文件名/失败原因），分享出去的每一次投递在 App 里都有明确归宿；⑤ **ZIP 管线明确化**：多个内嵌 IPA 时列出全部候选并明确"不随机选择"，零 IPA 的 ZIP 明确报"ZIP 中未找到 IPA 文件"。
- **v1.0.147**（激活规则键名修正 + 失败重试状态机版）：① 激活规则键名修正（关键）：字典规则的文件计数 key 必须是【单数 File】——`NSExtensionActivationSupportsFileWithMaxCount`（Apple 文档原文，"所有类型文件的最大数量"）；此前使用的复数 `FilesWithMaxCount` 是不存在的 key，系统按未声明处理，文件载荷永远无法命中激活规则（分享入口出现却收不到文件载荷的直接原因之一）。② 签名引擎（zsign）entitlements 对齐：扩展二进制此前原样携带主 App 的 application-identifier（TEAM.com.x.y ≠ 扩展 bundle id），现按扩展自身 bundle id 改写 id 部分（Team 前缀与 App Groups 不动，主 App 逐字节保持原样），与正规重签工具产物一致。③ 投递结算状态机重构：**成功才**写 processed/内容身份并删除源文件；失败保留源文件、只累计失败次数（60 秒节流防生命周期扫描连导 4 遍，连续 3 次失败后停止自动重试、源文件保留供手动处理）——旧版成功失败都结算删除，导入失败的文件从此失去重试机会。④ force 语义补全：URL 事件去重只拦非强制触发，系统主动投递的 open 事件（force=true）真正穿透去重（旧结构里 force 在函数最前面就被吞掉）。⑤ 落盘可靠性：扩展复制后校验目标存在、大于 0 字节且与源等大，半成品即抛错（绝不把未落盘当成功后结束请求）；扩展链路全程留痕（provider 载荷/临时文件/逐容器复制成败）。⑥ 产物校验闭环：签名完成后自动扫描产物 IPA 中央目录校验"每个扩展已嵌入描述文件"（缺失写入失败专区）；CI 增加扩展 Info.plist 校验（单数 File key + bundle id 子级关系）。
- **v1.0.146**（可见流水线 + 防重复投递版）：① 自动一条龙状态卡：签名阶段真实进度上屏（旧版空回调，大包 20-60 秒界面空白，用户以为卡死、切后台掐断流水线），加"正在发起安装/安装已发起"阶段；② 投递文件内容身份去重（文件名|大小|mtime 持久化），路径失效也能防重复导入；③ 同 bundleID 自动签名 5 分钟冷却期（防时间错开的重复导入连签两次）；④ 结算删除源文件失败留痕。
- **v1.0.145**（扩展描述文件修复版）：修复"点分享面板入口只跳到主 App、扩展毫无反应"的最终根因。① 签名引擎（zsign）补丁：`embedded.mobileprovision` 的写入条件原为"描述文件 application-identifier 以 bundle ID 结尾"——主 App 命中、扩展（com.x.y.Share 是主 App id 的**延伸**而非被延伸）永远不命中，扩展包内因此**没有描述文件**，iOS 17+ 直接拒绝加载（实测 iOS 27：入口显示但点按不呈现扩展 UI）。现在顶层 `PlugIns/*.appex` 增加回退匹配：描述文件 bundle 部分是扩展 id 前缀（或 `*` 通配）时同样写入描述文件，与正规重签工具（Xcode/Sideloadly）产物布局一致；② 扩展运行时组发现补宿主回退：扩展自身无描述文件时沿目录上溯读宿主主 App 的 embedded.mobileprovision（appex 就在主 App bundle 内，无沙盒外权限需求），组候选不再只剩"默认名+沙盒外扫描"两条死路；③ 分享/动作扩展激活规则 TRUEPREDICATE → 字典谓词；④ 诊断报告对"扩展内无描述文件"给出醒目警告与重签指引；投递日志页摘要卡新增备用通道指引（文件 App → IPA Manager 文件夹）。
- **v1.0.142**（投递链路重构版）：分享接收全链路重设计，修复"分享/跳转后毫无反应 + 扩展日志全空 + 扫描日志刷屏"三联问题。① 根因修复：恢复并强化"运行时从 embedded.mobileprovision 自动发现描述文件实际授予的 App Group"（v1.0.130 引入后曾被误删，替换成的"默认组名 → 本进程缓存 → 沙盒外扫描"三连在扩展进程内必然全部失败且静默——描述文件未授权默认组 × 扩展私有 UserDefaults 读不到主 App 缓存 × 扩展沙盒通常禁读沙盒外目录）；候选组来源合并为：默认声明组 + **描述文件授予组** + 本进程缓存 + 沙盒外扫描兜底。② 扇出/扇入架构：扩展保存文件与日志时写入**全部探针实测可写**的容器（containerURL 非 nil ≠ 可写，逐一写入-读回-删除实测），主 App 扫描收件箱/扩展日志时枚举全部可用容器——两个进程即使解析到不同组也不丢文件（旧版只扫单个解析组，组错位即全盲）。③ 扩展日志永不静默：组解析失败不再吞掉"扩展启动"（旧版日志写入被组解析门控，容器不可用时连"扩展启动"都写不出来，诊断全盲）；无容器可写时降级 NSLog，扩展 UI 直接展示完整诊断（声明组/授予组/逐组探针结果/解决办法），失败文件保留 12 秒可截图。④ 投递日志降噪：扫描改为"指纹变化才落一条"（容器集合 + 各目录计数；旧版每次回前台固定刷 2~4 条"无文件"，120 条环形缓冲约 20 次前后台切换刷光、真实信号全淹没，用户实测"日志每次都一模一样"的直接原因）；冷启动"无 URL"不再记；"共享容器可用"仅组集合变化才记。⑤ 日志与错误收集重做：失败专区跨启动持久化（旧版纯内存每次启动清零，报告永远"无失败记录"），扩展侧失败行升级错误级并进失败专区；投递日志页新增 立即扫描/复制全部/清空/过滤（全部·扩展·异常）与摘要卡（可用容器、扩展最近活动、最近投递、错误计数），数据实时绑定（旧版只在打开页面时拉一次，宣称"实时"却永不刷新）。⑥ 诊断报告新增「共享容器与投递通道自检」章节（组候选来源、逐组探针、各容器收件箱状态）；扩展日志缺失时给出按可能性排序的排查结论（点了"拷贝到"入口属正常 / 扩展被拒绝加载 / 容器整体不可用）。⑦ 投递扫描扩展到 Documents 子目录（有界深度、排除本 App 管理目录）：文件 App 是 iOS 27 上最可靠的投递通道，分享面板路由失效时"把文件存/移动到 IPA Manager 文件夹（含自建子文件夹）"仍能触发自动一条龙。⑧ 同轮修复 15+ 处：安装失败兜底 stop() 掐死上一个进行中安装、用户取消安装后服务器永不标记空闲（批量后续安装阻塞 10-15 分钟）、并发安装互杀监听器、手动签名自动回桌面未锚定 open 成功（后台 open 被忽略 → 静默失败）、下载页 zip 探测整包解压泄漏、描述文件重复导入不去重、孤儿清扫字符串前缀误匹配漏删、轻量解析不识别大写 .APP、安装证书选择器允许选过期证书等；构建侧：6 个 XcodeGen 生成文件入 .gitignore、扩展 target 排除对方 plist/entitlements 资源混入、CI 产物校验补 PlugIns 扩展存在性检查。
